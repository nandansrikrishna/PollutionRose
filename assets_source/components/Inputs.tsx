...
const Inputs: React.FC<inputsProps> = ({ propState }) => {
  ...
  const [subTimeStart, setSubTimeStart] = useState("00:00:00");
  // sub-interval end time
  const [subTimeEnd, setSubTimeEnd] = useState("23:59:59");
  // sub-interval days of the week
  const [subWeekDays, setSubWeekDays] = useState([
    true,
    true,
    true,
    true,
  ...
  const handleSelectChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (event.target.value === "7") {
      ...
      var endDate = new Date(
        ...
        Number(monthDayYear[0]) - 1,
        0
      );
      var startDate = new Date(
        Number(monthDayYear[2]),
        Number(monthDayYear[0]) - 2,
        1
      );
  ...
  const handleUnitsChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (calmRef.current) {
      calmRef.current.value = (
        Math.round(newCalmValue * 1000) / 1000
      ).toString();
    }
    ...
    <>
      <div className="input-wrapper">
        <form onSubmit={generateRose} onKeyDown={handleEnter}>
                ...
                <ul>
                  ...
                  <li>
                    <input
                      type="date"
                      className="textfield"
                      defaultValue={getDefaultDate(7)}
                      onChange={handleCustomPeriodStart}
                    ></input>
                    <span> - </span>
                    <input
                      type="date"
                      ...
                      onChange={handleCustomPeriodEnd}
                    ></input>
                  </li>
                </ul>
                ...
                <ul>
                  ...
                  <li>
                    <input
                      ...
                      className="textfield"
                      defaultValue="00:00"
                      onChange={handleCustomTimeStart}
                    />
                    <span> - </span>
                    <input
                      type="time"
          ...
          <div className="container">
            <button type="button" onClick={handleAdvancedParametersButton}>
              ...
              <div>
                <div className="calm-units">
                  ...
                  <input
                    ...
                    value="m/s"
                    onChange={handleUnitsChange}
                    checked={calmUnits === "m/s"}
                  />
                  <label>m/s</label>
                </div>
                <div>
                  <span>Calm Value: </span>
                  <input
                    ref={calmRef}
                    type="number"
                    min="0"
                    ...
                    className="textfield"
                    name="calm"
                    defaultValue={calmValue}
                    onBlur={handleCalmChange}
                  />
                </div>
              </div>
            )}
          ...
          <div className="container">
            ...
            {DEVMODE && (
              <p>
                ...
                measurement period beginning on {periodStart} and ending on{" "}
                {periodEnd},<br />
                subInterval Date beginning on {subDateStart} and ending on{" "}
                {subDateEnd},<br />
                subInterval Time beginning at {subTimeStart} and ending at{" "}
                {subTimeEnd},<br />
                on day(s) {subWeekDays[0] ? `monday, ` : ``}
                {subWeekDays[1] ? `tuesday, ` : ``}
                {subWeekDays[2] ? `wednesday, ` : ``}
                {subWeekDays[3] ? `thursday, ` : ``}
                {subWeekDays[4] ? `friday, ` : ``}
                {subWeekDays[5] ? `saturday, ` : ``}
                {subWeekDays[6] ? `sunday, ` : ``}
                <br />
